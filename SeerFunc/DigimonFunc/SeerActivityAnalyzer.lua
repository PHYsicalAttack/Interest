...
local Master_Ver = true                                              --是否使用主线,在更新版本时使用true.
local StartDate = "all"                                              --解析开始时间在该时间以后的活动,"all"|"auto"|2017.3.28"(all是解析所有活动,有开服活动的话用all,需要名称为all,auto的话自动会设定为下周一)
IsDigimon = true                                                     --数码宝贝专用:是否是数码宝贝
local DigimonBranchVer = "001"                                       --数码百倍专用:数码宝贝支线版本文件夹名字
---------以下内容无需修改-------------
...
local weekdir = comfunc.getItemByName(comfunc.strSplit(comfunc.getAllOfDIR("config")),StartDate)                --本周活动文件夹名称
if not weekdir then
