...
--解析函数:奖励 allrwd = {reward,{itemtb},{gertb},{equiptb},{null}}
function analfun:reward(allrwd)

	local rwd_t = allrwd
	function poorseer(t)
		local function isEquipType(itemid)
				local equip_min_id,equip_max_id = 2111,2575								   --赛尔装备类型id
				local crystal_min_id,crystal_max_id = 2895,2900							   --赛尔核心类型id 
				if (itemid >= equip_min_id and itemid <= equip_max_id) or (itemid >= crystal_min_id and itemid <= crystal_max_id) then
					return true
				end
				return false
			end
		...
		for i=1,#t[2] do 															--解析货币表
			...
			if  isEquipType(item_id) then 
				reward_item = item_id  .. " is not item type!"
				break
			end
		...
		for i=1,#t[4] do  															--解析装备表
			...
			if not isEquipType(equ_id) then 
				reward_equ = equ_id .. " is not equipment type!"
				break
			end
...
-- 许愿池
function  analfun:wish(t)
	local ver_id = t[1][2]
	local opentime = analfun:time(t[2][2])
	local pool_id = t[7][2]
	local pool_rwd = {}
	local line = 7
	for i = line+1,line + #pool_id do 
		table.insert(pool_rwd,t[i][2])
	end
	line = line +#pool_id
	local t_poolrwd = {}
	for i =1,#pool_id do
		local str_prob = string.format("概率:%-4s\32",pool_id[i][1])
		local str_rwd = analfun:reward(pool_rwd[i])
		table.insert(t_poolrwd,str_prob .. str_rwd)
	end
	local str_poolrwd  = table.concat(t_poolrwd,"\n")
	line = line+1	
	local fixbox = t[line][2]
	local fixtimes,fixrwd = {},{}
	for i = line+1 , line + #fixbox do
		table.insert(fixtimes,t[i][2][1])
		table.insert(fixrwd,t[i][2][2])
	end
	local tab_fixrwd = {}
	for i = 1,#fixtimes do
		local str_fixtimes= string.format("积分:%-4s\32",fixtimes[i])
		local str_fixrwd = analfun:reward(fixrwd[i])
		table.insert(tab_fixrwd,str_fixtimes .. str_fixrwd)
	end
	local str_fixdetail = table.concat(tab_fixrwd,"\n")
	line = line+#fixbox+1  				
	local wish_rank_len = t[line][2]	--排行榜容量
	line = line +2 						
	local tab_rank = t[line][2]				--reward_rank

	local tab_serial = {} 
	for num,tab in ipairs(tab_rank) do
		local startnum,endnum, num_rank = table.unpack(tab)
		if num ~= num_rank then 
			return "名次好像出错了?"
		end
		table.insert(tab_serial,string.format("第%2s档名次:%-2s~%2s%s",num_rank,startnum,endnum,string.rep("\32",4)))
	end 

	local tab_rankrwd= {}
	for i  = line+1,line+#tab_rank do
		table.insert(tab_rankrwd,t[i][2])
	end 
	local tab_rankdetail = {}
	for i = 1,#tab_serial do
		local serial = tab_serial[i]
		local str_rankrwd = analfun:reward(tab_rankrwd[i])
		table.insert(tab_rankdetail,serial .. str_rankrwd)
	end
	local str_rankdetail = table.concat(tab_rankdetail,"\n")
	local wish = string.format("###许愿池活动###\n活动ID:%s\n活动时间:%s\n###许愿池奖励###\n%s\n###保底奖励###\n%s\n###排行榜奖励###\n%s",ver_id,opentime,str_poolrwd,str_fixdetail,str_rankdetail)
	return wish
end
return analfun
