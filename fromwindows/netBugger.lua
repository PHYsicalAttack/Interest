...
function dealurl(url,keyword,startpage,endpage)
	...
	for i = startpage,endpage,20 do
		pageurl = string.format(url,keyword,startpage,endpage)
...
os.execute("mkdir ".. string.gsub(dir,"/","\\").. keyword)
for i,v in ipairs(dealurl(url,keyword,endpage)) do
	...
	if err == 200 then
		file:write(res)
		file:close()
		print("pay " .. endtime-pretime .. "s,download success!!")
	else
		print("pay" .. endtime-pretime .. "s,download from " .. v .." failed because of " .. err)
	end
end
